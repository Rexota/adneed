...
function reklam_ekle() {
 ...
 $('#reklam_ekle_alan').load('inc_reklam/msn.php?mode=new');
...
function odeme_bildirim(id) {
 ...
 $('#reklam_ekle_alan').load('inc_reklam/msn.php?mode=bildirim&id='+id);
...
function resimsec(id) {
 ...
 $('#reklam_ekle_alan').load('inc_reklam/msn.php?mode=resim1&id='+id);
...
function resimgoster(id) {
 ...
 $('#reklam_ekle_alan').load('inc_reklam/msn.php?mode=resimsee&id='+id);
 ...
 $.post('inc_reklam/msn.php?mode=sil', {id: id}, function(response) { });
 msn_goster();
...
<?	
$result = @mysql_query("select * from msn_reklamlar where reklamveren_id = '$_SESSION[userid]'");
...
$sor = @mysql_num_rows(mysql_query("select * from reklam_odeme where reklam = '$yaz[id]' and reklam_tur = 3"));
?>
<tr>
			<td height="36" width="25%" style="border: 1px solid #c0c0c0; padding-top: 1px">&nbsp;<a href="<?=msn_rapor($yaz[id])?>"><?=$yaz[url]?></a></td>
			...
			<td height="25" width="13%" style="border: 1px solid #c0c0c0; padding-top: 1px">&nbsp;<?=reklam_durum_3("$yaz[durum]","$yaz[id]")?></td>
...
$(function(){
	...
	$.post('inc_reklam/msn.php?mode=add', {istek: istek, sure : sure, url : url, cat : cat}, function(response) {
				...
				switch (return_val) {
					...
					case 'MIN_HIT':
						$('#islem_durum').html('<img src="extra/notification-exclamation.gif"> En az <?=number_format($msn_min_hit)?> Hit Alabilirsiniz !');
						break;
						
					case 'MAX_HIT':
						$('#islem_durum').html('<img src="extra/notification-exclamation.gif"> En fazla <?=number_format($msn_max_hit)?> Hit Alabilirsiniz !');
						break;
...
$.post('inc_reklam/msn.php?mode=hesap', {gelen : gelen}, function(response) {
	...
	<tr>
		<td height="35" colspan="2"><p style="width:390px; height:35px;line-height:35px;border-bottom: 1px solid #c0c0c0;font-size:13px; font-weight:bold;"><b>&nbsp;&nbsp;Msn Pop Reklam Ekleme Paneli</b></p></td>
	</tr>
	...
	<tr>
		...
		<td height="35" width="">&nbsp;<input type="text" class="ninput" name="fiyat" id="fiyat" disabled value="<?=$msnpopreklam?>">&nbsp;TL</td>
				</tr>
					<tr>
...
if ($istek < $msn_min_hit) {
die("MIN_HIT");
}

if ($istek > $msn_max_hit) {
die("MAX_HIT");
}
...
$yali = mysql_query("insert into msn_reklamlar (reklamveren_id, url, toplam, suan,ucret, gunluk, bugun, sure,kategori, gunluk_gosterim, tarih, durum, resim) values 
(
...
$del2 = @mysql_query("delete from reklam_odeme where reklam = '$id' and reklam_tur = 3");

$del = @mysql_query("Delete from msn_reklamlar where id = '$id' and durum=0");
...
$bul2 = mysql_fetch_array(mysql_query("select * from msn_reklamlar where id = '$rid'"));
...
$(function() {
   ...
   $('#bildir_button').click(function() {
	...
	$.post('inc_reklam/msn.php?mode=obildir', {rtur : rtur, gonderen : gonderen, banka : banka, odeme_turu : odeme_turu , not : not, rid : rid , hesap_numaramiz : hesap_numaramiz}, function(response) {
...
<? if ($mode=="resim1") { 
$id = sql($_GET['id']);
$yaz = @mysql_fetch_array(mysql_query("select * from msn_reklamlar where id = '$id' and reklamveren_id = '$_SESSION[userid]'"));
?>
<title></title>
<script type="text/javascript">
...
function urlden_yukle(id) {
 ...
 $('#reklam_ekle_alan').html('<img src="new/loadingAnimation.gif"> Veri Aktarılıyor ...');
 $('#reklam_ekle_alan').load('inc_reklam/msn.php?mode=url&id='+id);
...
function bilgisayardan_yukle(id) {
 ...
 $('#reklam_ekle_alan').html('<img src="new/loadingAnimation.gif"> Veri Aktarılıyor ...');
 $('#reklam_ekle_alan').load('inc_reklam/msn.php?mode=pc&id='+id);
...
$sor = @mysql_num_rows(mysql_query("select * from msn_reklamlar where id = '$id' and reklamveren_id = '$_SESSION[userid]'"));
if ($sor < 1) { die("FAILED"); } 
...
$ekle = mysql_query("UPDATE msn_reklamlar SET resim = '$resim' where id = '$id'");
...
$('#resim_kaydet_button').click(function() {
	...
	$.post('inc_reklam/msn.php?mode=urlimg', { resim : resim, id : id }, function(response) {
				...
				switch (return_val) {
					...
					case 'MIN_WIDTH':
					$('#resim_islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim genişliği <?=$msn_min_width?> PX den az olamaz !');
					break;					
					
					case 'MIN_HEIGHT':
					$('#resim_islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim yüksekliği <?=$msn_min_height?> PX den az olamaz !');
					break;						
						


					case 'MAX_WIDTH':
					$('#resim_islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim genişliği <?=$msn_max_width?> PX den fazla olamaz !');
					break;					
					
					case 'MAX_HEIGHT':
					$('#resim_islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim yüksekliği <?=$msn_max_height?> PX den fazla olamaz !');
					break;							
...
$('#resim_kaydet_button').click(function() {
	...
	$('#resim').upload('inc_reklam/msn.php?mode=pcimg&id=<?=$id?>', function(res) {
