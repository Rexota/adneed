...
$yalim = @mysql_num_rows(@mysql_query("select * from ppc_cat"));
if ($yalim < 1) {
...
function reklam_ekle() {
 ...
 $('#reklam_ekle_alan').load('inc_reklam/ppc.php?mode=new');
...
function odeme_bildirim(id) {
 ...
 $('#reklam_ekle_alan').load('inc_reklam/ppc.php?mode=bildirim&id='+id);
...
function resimsec(id) {
 ...
 $('#reklam_ekle_alan').load('inc_reklam/ppc.php?mode=resim1&id='+id);
...
function resimgoster(id) {
 ...
 $('#reklam_ekle_alan').load('inc_reklam/ppc.php?mode=resimsee&id='+id);
 ...
 $.post('inc_reklam/ppc.php?mode=sil', {id: id}, function(response) { });
 ppc_banner_goster();
...
<table border="0" width="100%" cellpadding="0" style="border-collapse: collapse">
		...
		<tr>
			<td height="25" width="20%" style="border: 1px solid #C0C0C0">&nbsp;Reklam Url</td>
			<td height="25" width="14%" style="border: 1px solid #C0C0C0">&nbsp;Resim</td>
			<td height="25" width="9%" style="border: 1px solid #C0C0C0">&nbsp;Kategori</td>
			<td height="25" width="10%" style="border: 1px solid #C0C0C0">&nbsp;Durum</td>
			<td height="25" width="10%" style="border: 1px solid #C0C0C0">&nbsp;Bütçe</td>
			<td height="25" width="10%" style="border: 1px solid #C0C0C0">&nbsp;Kalan Bütçe</td>
			<td height="25" width="27%" style="border: 1px solid #C0C0C0">&nbsp;Gösterilme durumu yada İşlemler</td>
		</tr>
...
<?	
$result = @mysql_query("select * from ppc_reklamlar where reklamveren_id = '$_SESSION[userid]'");

if (@mysql_num_rows($result)<1) {
?>

		<tr>
			<td height="32" width="100%" colspan="7" style="border: 1px solid #c0c0c0">&nbsp;<img border="0" src="extra/notification-exclamation.gif">&nbsp;Şuan gösterimde olan yada biten bir reklamınız yok !</td>
		</tr>
...
$sor = @mysql_num_rows(mysql_query("select * from reklam_odeme where reklam = '$yaz[id]' and reklam_tur = 4"));

$kalan = $yaz[tikbas]*$yaz[suan];
$kalan = $yaz[ucret]-$kalan;

?>
<tr>
			<td height="36" width="20%" style="border: 1px solid #c0c0c0; padding-top: 1px">&nbsp;<a href="<?=ppc_rapor($yaz[id])?>"><?=$yaz[url]?></a></td>
			<td height="25" width="14%" style="border: 1px solid #C0C0C0"><? if ($yaz['resim']=='') { ?>&nbsp;<input type="button" onclick="resimsec(<?=$yaz[id]?>);" class="button" value="Resim Yükleyin"><? } else { ?>&nbsp;<input type="button" onclick="resimgoster(<?=$yaz[id]?>);" class="button" value="Resimi Görüntüle"><? } ?></td>
			<td height="25" width="9%" style="border: 1px solid #c0c0c0; padding-top: 1px">&nbsp;<?=reklam_kategori($yaz[kategori])?></td>
			<td height="25" width="10%" style="border: 1px solid #c0c0c0; padding-top: 1px">&nbsp;<?=reklam_durum_4("$yaz[durum]","$yaz[id]")?></td>
			<td height="25" width="10%" style="border: 1px solid #c0c0c0; padding-top: 1px">&nbsp;<?=number_format($yaz[ucret],2)?> TL</td>
			<td height="25" width="10%" style="border: 1px solid #c0c0c0; padding-top: 1px">&nbsp;<?=number_format($kalan,2)?> TL</td>
			<? if ($yaz[durum]=="0") { ?>
			<td height="25" width="27%" style="border: 1px solid #c0c0c0; padding-top: 1px"><p align="center"><? if ($_SESSION['tur']=="2") { ?><? if ($sor < 1 ) { ?><input type="button" onclick="odeme_bildirim(<?=$yaz[id]?>);" class="button" value="Ödeme Bildirimi Yap"><? } ?><? } ?>&nbsp;<input type="button" onclick="reklamsil(<?=$yaz[id]?>);" class="button" value="Reklamı Silin"></p></td>
            <? } else { ?>
            <td height="25" width="27%" style="border: 1px solid #c0c0c0; padding-top: 1px"><div style="font-size:12px;font-weight:bold;height:25px;line-height:25px;"><p align="center">%<?=$reklamyuzdesi?> - <?=number_format($yaz[toplam])?>/<?=number_format($yaz[suan])?></p></div></td>
            <? } ?>
...
$(function(){
	...
	$.post('inc_reklam/ppc.php?mode=add1', {mod : mod, cat : cat}, function(response) {
				...
				switch (return_val) {
					...
					case 'ERROR':
						$('#islem_durum').html('<img src="extra/notification-slash.gif"> Kategori Yada Modül Seçmediniz !');
						break;
						
					case 'RETURN':
						$('#islem_durum').html('<img src="extra/notification-information.gif"> İşlem devam ediyor . . .');
						setTimeout("$('#reklam_ekle_alan').load('inc_reklam/ppc.php?mode=new2');", 1500);
						break;
	...
	<tr>
		<td height="35" colspan="2"><p style="width:390px; height:35px;line-height:35px;border-bottom: 1px solid #c0c0c0;font-size:13px; font-weight:bold;"><b>&nbsp;&nbsp;PPC Reklam Ekleme Paneli</b></p></td>
	</tr>
	...
	<tr>
		<td height="20" colspan="2"></td>
					...
					<tr>
		<td height="35" width="15%">&nbsp;Reklam Modülü:</td>
		<td height="35" width="">&nbsp;<select name="mod" id="mod" size="1" class="select">
		<?	
$result = @mysql_query("select * from ppc_cat");

if (mysql_num_rows($result)<1) {
?>

<option value="0">Modül Bulunamadı !</option>	
<? 
} else {
...
?>
<option value="<?=$yaz[id]?>"><?=$yaz[ad]?> - <?=$yaz[width]?>x<?=$yaz[height]?></option>
...
<? } 
		...
		<td height="35" width="" align="left">&nbsp;<input type="button" name="al_button" id="al_button" class="button" value="Devam Et">&nbsp;<input onclick="reklam_ekle_kapat();" type="button" class="button" value="Vazgeç"></td>
					</tr>
					<tr>
		<td height="25" colspan="2">&nbsp;<span id="islem_durum"></span></td>
					</tr>
...
if ($cat=='' or $cat=='0' and $mod=='' or $mod=='0') { die("ERROR"); }

$bul = @mysql_num_rows(@mysql_query("select * from ppc_cat where id = '$mod'"));
$sor = @mysql_num_rows(@mysql_query("select * from kategoriler where id = '$cat'"));

if ($bul < 1 or $sor < 1) { die("ERROR"); } Else { 
$_SESSION['ppc_mode'] = $mod;
$_SESSION['ppc_cat'] = $cat;
die("RETURN");
 }
...
$(function(){
	...
	var tiksay = $('#tiksay').val();
	var butcem = $('#butcem').val();
	var sure = $('#sure').val();
	var url = $('#url').val();
	
	

	$.post('inc_reklam/ppc.php?mode=add2', { tiksay : tiksay, butcem: butcem, sure: sure, url : url }, function(response) {
				...
				switch (return_val) {
					...
					case 'SUCCESS':
				    $('#islem_durum').html('<img src="extra/notification-information.gif"> Reklamınız Eklendi. Lütfen Bekleyin . . . ');
					setTimeout("ppc_banner_goster();", 1500);	
						break;
	...
	<?
	$mod = $_SESSION['ppc_mode'];
	$cat = $_SESSION['ppc_cat'];
	
	$mbul = @mysql_fetch_array(@mysql_query("select * from ppc_cat where id = '$mod'"));
	$cbul = @mysql_fetch_array(@mysql_query("select * from kategoriler where id = '$cat'"));
	?>
	
		<tr>
			<td height="35" width="15%">&nbsp;Modül:</td>
			<td height="25" width="">&nbsp;<b><?=$mbul['ad']?> - <?=$mbul['width']?>x<?=$mbul[height]?> PX</b></td>
		</tr>
		
		<tr>
			<td height="35" width="">&nbsp;Kategori:</td>
			<td height="25" width="">&nbsp;<b><?=$cbul[name]?></b></td>
		</tr>
		
		
		
		<tr>
				<td height="35" width="">&nbsp;Tık Başı Ücret:</td>
			<td height="25" width="">&nbsp;<input size="4" name="" id="tiksay" type="text" class="input" value="<?=$mbul['min']?>"  onkeypress="return BlockNonNumbers(this, event,'.',true,false);" maxlength="6"   /> TL   (Tık başı ücreti yüksek olan reklamlar daha çok gösterilir)</td>
		</tr>
		
		
			
		<tr>
			<td height="35" width="">&nbsp;Bütçeniz:</td>
			<td height="25" width="">&nbsp;<input id="butcem" size="15" value="<?=number_format($mbul['min_tutar'],2)?>" onblur="ExtractNumber(this,'.',',',2,false);" onkeyup="ExtractNumber(this,'.',',',2,false);" onkeypress="return BlockNonNumbers(this, event,'.',true,false);" name="" type="text" class="input" /> TL
			</td>
		</tr>
		...
		<td height="35" width="">&nbsp;<input id="url" name="url" type="text" class="ninput"></td>
				...
				<tr>
		<td height="35" width="">&nbsp;İşlem</td>
		<td height="35" width="" align="left">&nbsp;<input type="button" name="al_button" id="al_button" class="button" value="Satın Al">&nbsp;<input onclick="reklam_ekle_kapat();" type="button" class="button" value="Vazgeç"></td>
					</tr>
					<tr>
...
$sure = sql($_POST['sure']);
$url = sql($_POST['url']);
$butcem = sql($_POST['butcem']);
$tiksay = sql($_POST['tiksay']);

	$mbul = @mysql_fetch_array(@mysql_query("select * from ppc_cat where id = '$mod'"));
	$cbul = @mysql_fetch_array(@mysql_query("select * from kategoriler where id = '$cat'"));


if ($mod=='' or $cat=='' or $sure=='' or $butcem=='' or $tiksay=='' or $url=='') { die("ERROR"); }

$butcem = str_replace(",","",$butcem);
$tarih = mktime();
...
tiku_kontrol("$tiksay","<img src='extra/notification-exclamation.gif'>","Tık Ücreti");

if ($tiksay < $mbul[min]) {
die("<img src='extra/notification-exclamation.gif'> Tık Başı Ücretiniz $mbul[min] TL den az Olamaz !");
}

if ($tiksay > $mbul[max]) {
die("<img src='extra/notification-exclamation.gif'> Tık Başı Ücretiniz $mbul[max] TL den Fazla Olamaz !");
}

if ($butcem < $mbul[min_tutar]) {
die("<img src='extra/notification-exclamation.gif'> Bütçeniz ".number_format($mbul[min_tutar],2)." TL den az Olamaz !");
}

if ($butcem > $mbul[max_tutar]) {
die("<img src='extra/notification-exclamation.gif'> Bütçeniz ".number_format($mbul[max_tutar],2)." TL den Fazla Olamaz !");
}
...
$yali = mysql_query("insert into ppc_reklamlar (reklamveren_id, url, toplam, suan,ucret, gunluk, bugun, sure,kategori, gunluk_gosterim, tarih, durum, resim, modul, tikbas) values 
(
'$_SESSION[userid]', '$url', '$istek', '0', '$butcem', '$gunluk', '0', '$sureg', '$cat', '0', '$tarih', '0', '', '$mod', '$tiksay'
)") or die(mysql_error());
...
$del2 = @mysql_query("delete from reklam_odeme where reklam = '$id' and reklam_tur = 4");

$del = @mysql_query("Delete from ppc_reklamlar where id = '$id' and durum=0");
...
$bul2 = mysql_fetch_array(mysql_query("select * from ppc_reklamlar where id = '$rid'"));
...
$yaz = @mysql_fetch_array(mysql_query("select * from ppc_reklamlar where id = '$id'"));
?>
<title></title>
...
$(function() {
   ...
   $('#bildir_button').click(function() {
	...
	$.post('inc_reklam/ppc.php?mode=obildir', {rtur : rtur, gonderen : gonderen, banka : banka, odeme_turu : odeme_turu , not : not, rid : rid , hesap_numaramiz : hesap_numaramiz}, function(response) {
...
<? if ($mode=="resim1") { 
$id = sql($_GET['id']);
$yaz = @mysql_fetch_array(mysql_query("select * from ppc_reklamlar where id = '$id' and reklamveren_id = '$_SESSION[userid]'"));
?>
<title></title>
<script type="text/javascript">
...
function urlden_yukle(id) {
 ...
 $('#reklam_ekle_alan').html('<img src="new/loadingAnimation.gif"> Veri Aktarılıyor ...');
 $('#reklam_ekle_alan').load('inc_reklam/ppc.php?mode=url&id='+id);
...
function bilgisayardan_yukle(id) {
 ...
 $('#reklam_ekle_alan').html('<img src="new/loadingAnimation.gif"> Veri Aktarılıyor ...');
 $('#reklam_ekle_alan').load('inc_reklam/ppc.php?mode=pc&id='+id);
...
$eh = mysql_query("select * from ppc_reklamlar where id = '$id' and reklamveren_id = '$_SESSION[userid]'");
$cibabam = @mysql_fetch_array($eh);
$sor = @mysql_num_rows($eh);
if ($sor < 1) { die("FAILED"); } 

$bul = @mysql_fetch_array(mysql_query("select * from ppc_cat where id = '$cibabam[modul]'"));

$uzanti = substr($resim ,-3);
if ($uzanti == "SWF" || $uzanti == "swf" || $uzanti == "jpg" || $uzanti == "JPG" || $uzanti == "gif" || $uzanti == "GIF" || $uzanti == "png" || $uzanti == "PNG") { } else {
die("FORMAT");
}

if ($uzanti=="SWF" || $uzanti=="swf") {} else {
...
if ($en != $bul[width] or $boy != $bul[height]) { die("BIG");  }

}


$base = rand(9999,9999999);
$code = "ppc_banner_ads_".md5($base);
...
$ekle = mysql_query("UPDATE ppc_reklamlar SET resim = '$resim' where id = '$id'");
...
$('#resim_kaydet_button').click(function() {
	...
	$.post('inc_reklam/ppc.php?mode=urlimg', { resim : resim, id : id }, function(response) {
				...
				switch (return_val) {
					...
					case 'BIG':
					$('#resim_islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim Boyutları <?=$bul[width]?>x<?=$bul[height]?> PX Olmalıdır !');
					break;				
	...
	<tr>
		<td height="20" colspan="2"></td>
						...
						<tr>
							<td height="30" width="">&nbsp;Resim Boyutu:</td>
							<td height="35" width=""><b><?=$bul[width]?>x<?=$bul[height]?> PX</b></td>
						</tr>
						
						
						<tr>
							<td height="30" width="">&nbsp;Desteklenen Türler:</td>
							<td height="35" width=""><b>SWF, JPG , GIF , PNG</b></td>
						</tr>
...
$('#resim_kaydet_button').click(function() {
	...
	$('#resim').upload('inc_reklam/ppc.php?mode=pcimg&id=<?=$id?>', function(res) {
...
<?
$uzanti = substr($yaz[resim] ,-3);
?>

<? if ($uzanti=="SWF" || $uzanti=='swf') { ?>
<td height="35" width="">&nbsp;					
<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,0,0" id="FullScreenTest" align="middle">
<param name="allowScriptAccess" value="sameDomain" />
<param name="allowFullScreen" value="true" />
<param name="movie" value="<?=$yaz[resim]?>" />
<param name="menu" value="false" />
<param name="quality" value="high" />
<param name="bgcolor" value="#ffffff" />
<embed src="<?=$yaz[resim]?>" menu="false" quality="high" bgcolor="#ffffff" name="vidyo" align="middle" allowScriptAccess="sameDomain" allowFullScreen="true" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" />
</object>
</td>
<? } else { ?>
