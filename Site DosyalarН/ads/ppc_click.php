...
$aysip = $_COOKIE["ppc_click_user_ip"];

if ($aysip=='') {
$ip = GetIP();
setcookie("ppc_click_user_ip",$ip, time()+$havuz);
} else { 
$ip = $_COOKIE["ppc_click_user_ip"];
}
...
$dosya = "cache_ppc/".$reklam_benzersiz.".txt";
$icerik = @file($dosya);

if (!$icerik) { @header("Location: $_SESSION[ppc_reklam_adresi]"); } else {
...
$kst2 = explode("[/kesinti]", $kst1[1]);
$reklam_kesinti = $kst2[0];

$rtk1 = explode("[tikbas]", $buldum);
$rtk2 = explode("[/tikbas]", $rtk1[1]);
...
$dosyam = "click_ppc/".$reklam_benzersiz.".txt";
...
$verim = $verim."[kesinti]".$reklam_kesinti."[/b]";
$verim = $verim."[tikbas]".$reklam_tikbas."[/b]";
$dt = fopen($dosyam, 'w');
fwrite($dt, $verim);
fclose($dt);
...
@unlink($dosya);
$_SESSION['ppc_reklam_adresi'] = $reklam_adresi;
