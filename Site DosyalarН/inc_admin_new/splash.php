...
$('#reklamlar').load('inc_admin_new/splash.php?mode=tumu');
...
$('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor ...');
$('#reklamlar').load('inc_admin_new/splash.php?mode=biten');
...
$('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor ...');
$('#reklamlar').load('inc_admin_new/splash.php?mode=yayinda');
...
$('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor ...');
$('#reklamlar').load('inc_admin_new/splash.php?mode=bekleyen');
...
function sayfa_getir(id) {
 $('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor...');
 $('#reklamlar').load('inc_admin_new/splash.php?mode=<?=$mode?>&p='+id);
}

 function reklamsil(id) {
 
 $.post('inc_admin_new/splash.php?mode=sil', {id: id}, function(response) {
 $('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor...');
 $('#reklamlar').load('inc_admin_new/splash.php?mode=<?=$mode?>&p='+id);	
	 });
  ...
  $('#reklam_duzen_panel').load('inc_admin_new/splash.php?mode=edit&id='+id);
 }
...
function resimgoster(id) {
 ...
 $('#reklam_duzen_panel').load('inc_admin_new/splash.php?mode=resimsee&id='+id);
}
...
if ($mode=='tumu') { $count      = @mysql_num_rows(mysql_query("select * from splash_reklamlar")); }
if ($mode=='yayinda') { $count      = @mysql_num_rows(mysql_query("select * from splash_reklamlar where durum = 1")); }
if ($mode=='bekleyen') { $count      = @mysql_num_rows(mysql_query("select * from splash_reklamlar where durum = 0")); }
if ($mode=='biten') { $count      = @mysql_num_rows(mysql_query("select * from splash_reklamlar where durum = 2")); }

$toplamsayfa    = ceil($count / $limit);
$baslangic  = ($git-1)*$limit;


if ($mode=='tumu') { $result = @mysql_query("select * from splash_reklamlar LIMIT $baslangic,$limit"); }
if ($mode=='yayinda') { $result = @mysql_query("select * from splash_reklamlar where durum = 1 LIMIT $baslangic,$limit"); }
if ($mode=='bekleyen') { $result = @mysql_query("select * from splash_reklamlar where durum = 0 LIMIT $baslangic,$limit"); }
if ($mode=='biten') { $result = @mysql_query("select * from splash_reklamlar where durum = 2 LIMIT $baslangic,$limit"); }
...
?>
		    ...
		    <tr>
			<td height="25" width="20%" style="border: 1px solid #C0C0C0">&nbsp;<a href="<?=admin_splash_rapor($yaz[id])?>"><?=$yaz[url]?></a></td>
...
$del =  mysql_query("delete from splash_reklamlar where id = '$id'");

$del1 = mysql_query("delete from reklam_odeme where reklam = '$id' and reklam_tur = 2");
...
$bbul = @mysql_fetch_array(mysql_query("select * from splash_reklamlar where id = '$id'"));
$istek = $bbul[toplam];
...
$sql = "UPDATE splash_reklamlar SET url = '$url', sure = '$sureg', gunluk = '$gunluk', durum = '$durum', kategori = '$cat', resim = '$resim' where id = '$id'";
...
$(function(){
	...
	$.post('inc_admin_new/splash.php?mode=save', {id : id , durum : durum, sure : sure, url :  url, cat : cat, resim : resim}, function(response) {
				...
				switch (return_val) {
					...
					case 'MIN_WIDTH':
					$('#islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim genişliği <?=$splash_min_width?> PX den az olamaz !');
					break;					
					
					case 'MIN_HEIGHT':
					$('#islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim yüksekliği <?=$splash_min_height?> PX den az olamaz !');
					break;						
						


					case 'MAX_WIDTH':
					$('#islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim genişliği <?=$splash_max_width?> PX den fazla olamaz !');
					break;					
					
					case 'MAX_HEIGHT':
					$('#islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim yüksekliği <?=$splash_max_height?> PX den fazla olamaz !');
					break;	
