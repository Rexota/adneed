...
$('#reklamlar').load('inc_admin_new/ppc.php?mode=tumu');
...
$('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor ...');
$('#reklamlar').load('inc_admin_new/ppc.php?mode=biten');
...
$('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor ...');
$('#reklamlar').load('inc_admin_new/ppc.php?mode=yayinda');
...
$('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor ...');
$('#reklamlar').load('inc_admin_new/ppc.php?mode=bekleyen');
...
function sayfa_getir(id) {
 $('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor...');
 $('#reklamlar').load('inc_admin_new/ppc.php?mode=<?=$mode?>&p='+id);
}

 function reklamsil(id) {
 
 $.post('inc_admin_new/ppc.php?mode=sil', {id: id}, function(response) {
 $('#reklamlar').html('<img src="new/loadingAnimation.gif"> Yükleniyor...');
 $('#reklamlar').load('inc_admin_new/ppc.php?mode=<?=$mode?>&p='+id);	
	 });
  ...
  $('#reklam_duzen_panel').load('inc_admin_new/ppc.php?mode=edit&id='+id);
 }
...
function resimgoster(id) {
 ...
 $('#reklam_duzen_panel').load('inc_admin_new/ppc.php?mode=resimsee&id='+id);
}
...
if ($mode=='tumu') { $count      = @mysql_num_rows(mysql_query("select * from ppc_reklamlar")); }
if ($mode=='yayinda') { $count      = @mysql_num_rows(mysql_query("select * from ppc_reklamlar where durum = 1")); }
if ($mode=='bekleyen') { $count      = @mysql_num_rows(mysql_query("select * from ppc_reklamlar where durum = 0")); }
if ($mode=='biten') { $count      = @mysql_num_rows(mysql_query("select * from ppc_reklamlar where durum = 2")); }

$toplamsayfa    = ceil($count / $limit);
$baslangic  = ($git-1)*$limit;


if ($mode=='tumu') { $result = @mysql_query("select * from ppc_reklamlar LIMIT $baslangic,$limit"); }
if ($mode=='yayinda') { $result = @mysql_query("select * from ppc_reklamlar where durum = 1 LIMIT $baslangic,$limit"); }
if ($mode=='bekleyen') { $result = @mysql_query("select * from ppc_reklamlar where durum = 0 LIMIT $baslangic,$limit"); }
if ($mode=='biten') { $result = @mysql_query("select * from ppc_reklamlar where durum = 2 LIMIT $baslangic,$limit"); }
...
?>
		    ...
		    <tr>
			<td height="25" width="20%" style="border: 1px solid #C0C0C0">&nbsp;<a href="<?=admin_ppc_rapor($yaz[id])?>"><?=$yaz[url]?></a></td>
...
$del =  mysql_query("delete from ppc_reklamlar where id = '$id'");

$del1 = mysql_query("delete from reklam_odeme where reklam = '$id' and reklam_tur = 4");
...
$bbul = @mysql_fetch_array(mysql_query("select * from ppc_reklamlar where id = '$id'"));
$istek = $bbul[toplam];
...
$bul = @mysql_fetch_array(mysql_query("select * from ppc_cat where id = '$bbul[modul]'"));
...
$sql = "UPDATE ppc_reklamlar SET url = '$url', sure = '$sureg', gunluk = '$gunluk', durum = '$durum', kategori = '$cat', resim = '$resim' where id = '$id'";
...
$(function(){
	...
	$.post('inc_admin_new/ppc.php?mode=save', {id : id , durum : durum, sure : sure, url :  url, cat : cat, resim : resim}, function(response) {
				...
				switch (return_val) {
					...
					case 'BIG':
					$('#islem_durum').html('<img src="extra/notification-exclamation.gif"> Resim Boyutları <?=$bul[width]?>x<?=$bul[height]?> PX Olmalıdır !');
					break;	
	...
	<tr>
		<td height="45" colspan="2"><p style="width:390px; height:35px;line-height:35px;border-bottom: 1px solid #c0c0c0;font-size:13px; font-weight:bold;"><b>&nbsp;&nbsp;Reklam Düzenleme Paneli</b></p></td>
							...
							<tr>
							<td height="30" width="">&nbsp;Reklam Modülü:</td>
							<td height="35" width=""><b><?=$bul[ad]?></b></td>
						</tr>
						
						<tr>
							<td height="30" width="">&nbsp;Tık Başı:</td>
							<td height="35" width=""><b><?=$yaz[tikbas]?> TL</b></td>
						</tr>
						
						<tr>
							<td height="30" width="">&nbsp;Bütçe:</td>
							<td height="35" width=""><b><?=number_format($yaz[ucret],2)?> TL</b></td>
						</tr>
